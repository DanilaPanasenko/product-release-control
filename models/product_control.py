...
class BatchModel(Base):
    ...
    shift_start_datetime: Mapped[datetime] = mapped_column(DateTime(timezone=True), comment="Дата время начала смены")
    shift_end_datetime: Mapped[datetime] = mapped_column(DateTime(timezone=True), comment="Дата время конца смены")
    ...
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now(), comment="Дата создания записи")
    ...
    products = relationship("ProductModel", back_populates="batch", cascade="all, delete-orphan")
...
class ProductModel(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    unique_code: Mapped[str] = mapped_column(unique=True, comment="Уникальный код продукта")
    batch_id: Mapped[int] = mapped_column(ForeignKey("batches.id"))
    is_aggregated: Mapped[bool] = mapped_column(default=False, comment="Флаг агрегации")
    aggregated_at: Mapped[datetime] = mapped_column(DateTime, nullable=True, comment="Дата агрегации")
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now(), comment="дата создания записи")
